    ...
    const poll = async () => {
      try {
        ...
        const res = await ky.get(
          `${process.env.NEXT_PUBLIC_API_URL}/topview/v1/video_avatar/task/query?taskId=${taskId}`,
          {
            headers: {
        ...
        const { status, outputVideoUrl, errorMsg } = response.result;
        ...
        switch (status) {
          ...
          case "failed":
            reject(new Error(errorMsg || "Video generation failed"));
            break;
