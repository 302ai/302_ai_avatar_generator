      ...
      try {
        ...
        if (statusResp.status === "completed") {
          return {
            ...
            status: "success",
            data: {
              id: statusResp.result?.video_url,
            },
            audio_path: statusResp.result?.video_url,
            video_url: statusResp.result?.video_url,
        ...
        // 如果还在处理中且未超过最大重试次数，继续轮询（最多轮询50次）
        ...
        logger.error(
          `Voice clone status API call failed on attempt ${attemptCount + 1}:`,
          apiError
        );
        ...
        if (attemptCount < 1) {
          console.log(
            `Retrying merge video status check (attempt ${attemptCount + 2}/2)`
          );
          await new Promise((resolve) => setTimeout(resolve, 5000));
          return pollStatus(attemptCount + 1);
        }
        ...
        // 重试1次后仍失败，抛出错误
        console.error(
          `Merge video status check failed after ${attemptCount + 1} attempts`
        );
        throw apiError;
      }
    };
