...
interface CreateTopviewVideoParams {
  apiKey: string;
  videoUrl: string;
  audioUrl: string;
}

interface CreateTopviewVideoResult {
  taskId: string;
}

export const createTopviewVideo = async ({
  apiKey,
  videoUrl,
  audioUrl,
}: CreateTopviewVideoParams) => {
  try {
    const res = await ky.post<CreateTopviewVideoResult>(
      "/api/create-topview-video",
      {
        timeout: false,
        json: {
          apiKey,
