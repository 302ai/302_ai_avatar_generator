...
interface CreateAudioResult {
  data: {
    id: string;
  };
}

export const createAudio = async ({ apiKey, audioUrl }: CreateAudioParams) => {
  try {
    const res = await ky.post("/api/create-audio", {
      timeout: false,
      json: {
        audioUrl,
        apiKey,
    ...
    return res.json<CreateAudioResult>();
  } catch (error) {
